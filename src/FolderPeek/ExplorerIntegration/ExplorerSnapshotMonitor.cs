...
namespace FolderPeek.ExplorerIntegration;
...
internal sealed class ExplorerSnapshotMonitor : IDisposable
{
    ...
    {
        _thread = new Thread(Run) { IsBackground = true, Name = "FolderPeek Explorer monitor" };
    ...
    private void Run()
    {
        while (!_stop.IsCancellationRequested)
        {
            ...
            {
                Debug.WriteLine($"FolderPeek monitor recovered: {exception}");
    ...
    private ExplorerSnapshot Capture()
    {
        ...
        {
            Debug.WriteLine($"FolderPeek snapshot: {exception.Message}");
