...
internal static class DiagnosticsLog
{
    ...
    {
        var configured = Environment.GetEnvironmentVariable("FOLDERGLIMPSE_DIAGNOSTICS_PATH");
        _path = string.IsNullOrWhiteSpace(configured)
            ? System.IO.Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "FolderGlimpse", "diagnostics.log")
            : System.IO.Path.GetFullPath(configured);
        try
        {
            Directory.CreateDirectory(System.IO.Path.GetDirectoryName(_path)!);
            File.WriteAllText(_path, $"FolderGlimpse diagnostics started {DateTimeOffset.Now:O}{Environment.NewLine}");
        }
        catch { _path = null; }
    }
